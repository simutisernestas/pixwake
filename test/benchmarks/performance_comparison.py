...
def get_pywake_n_cpu(n_turbines, max_cpu=32):
    ...
    n_cpu = 2 + (max_cpu - 2) * (n_turbines - 50) / (1000 - 50)
    return max(2, min(max_cpu, int(np.round(n_cpu))))


def run_benchmark(n_turbines_list, spacings_list):
    """Runs the full performance benchmark."""
    # fmt: off
    ct_vals = np.array([0.80, 0.79, 0.77, 0.75, 0.72, 0.68, 0.64, 0.62, 0.61, 0.60, 0.55, 0.50, 0.45, 0.40, 0.35, 0.30, 0.25, 0.20, 0.18, 0.15, 0.12, 0.10, 0.10])
    power_vals = np.array([100, 300, 600, 1200, 1800, 2300, 2700, 2900, 2950, 3000, 3000, 3000, 3000, 3000, 3000, 3000, 3000, 3000, 3000, 3000, 3000, 3000, 0])
    # fmt: on
    ...
    for spacing in spacings_list:
        for n_turbines in n_turbines_list:
            ...
            pixwake_turbine = Turbine(rotor_diameter=rotor_diameter, hub_height=hub_height, power_curve=power_curve, ct_curve=ct_curve)
            pixwake_model = NOJModel(k=wake_expansion_k)
            pixwake_sim = WakeSimulation(pixwake_model, fpi_damp=1.0)

            pywake_power_ct = PowerCtTabular(ws=ct_pw_ws, power=power_vals * 1000, power_unit="w", ct=ct_vals)
            pywake_turbines = WindTurbines(names=[f"WT{i}" for i in range(n_turbines_actual)], diameters=[rotor_diameter] * n_turbines_actual, hub_heights=[hub_height] * n_turbines_actual, powerCtFunctions=[pywake_power_ct] * n_turbines_actual)
            pywake_wfm = All2AllIterative(site=UniformSite(), windTurbines=pywake_turbines, wake_deficitModel=NOJDeficit(k=wake_expansion_k))
            n_cpu = get_pywake_n_cpu(n_turbines_actual)

            ws_ts, wd_ts = generate_time_series_wind_data()
            start = time.time()
            pywake_wfm(x=x, y=y, wd=wd_ts, ws=ws_ts, n_cpu=n_cpu).aep().sum()
            pywake_aep_time_ts = time.time() - start
            start = time.time()
            pywake_wfm.aep_gradients(x=x, y=y, wd=wd_ts, ws=ws_ts, n_cpu=n_cpu)
            ...
            start = time.time()
            pywake_wfm(x=x, y=y, wd=wd_wr, ws=ws_wr, n_cpu=n_cpu).aep(freq=probs_wr).sum()
            pywake_aep_time_wr = time.time() - start
            start = time.time()
            pywake_wfm.aep_gradients(x=x, y=y, wd=wd_wr, ws=ws_wr, n_cpu=n_cpu, freq=probs_wr)
            pywake_grad_time_wr = time.time() - start

            @jax.jit
            def pixwake_aep_wr(xx, yy):
                return pixwake_sim(xx, yy, ws_wr, wd_wr, pixwake_turbine).aep(probabilities=probs_wr)
            ...
            results.append({
                ...
                "pywake_aep_time_wr": pywake_aep_time_wr,
                "pixwake_aep_time_wr": pixwake_aep_time_wr,
                "pywake_grad_time_wr": pywake_grad_time_wr,
                "pixwake_grad_time_wr": pixwake_grad_time_wr,
            })
            print(f"    PixWake Time-series (AEP/Grad): {pixwake_aep_time_ts:.4f}s / {pixwake_grad_time_ts:.4f}s")
            print(f"    PyWake Time-series (AEP/Grad): {pywake_aep_time_ts:.4f}s / {pywake_grad_time_ts:.4f}s")
...
def plot_results(results):
    ...
    for spacing in spacings:
        ...
        for name, (pywake_col, pixwake_col) in plot_configs.items():
            ...
            plt.plot(df_spacing["n_turbines"], df_spacing[pywake_col], "o-", label="PyWake")
            plt.plot(df_spacing["n_turbines"], df_spacing[pixwake_col], "o-", label="PixWake")
            plt.xlabel("Number of Turbines")
            plt.ylabel("Runtime (s)")
            plt.title(f"{name.replace('_', ' ')} Runtime Comparison ({spacing}D Spacing)")
            plt.legend()
            plt.grid(True, which="both", ls="--")
            plt.yscale("log")
            filename = f"benchmark_{name.lower()}_{spacing}D.png"
...
if __name__ == "__main__":
    ...
    N_TURBINES_LIST = [50, 100, 250, 500, 750, 1000]
    SPACINGS_LIST = [3, 5, 7]
