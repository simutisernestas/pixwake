...
def test_ws2power():
    ...
    actual_power = ws2power(ws_eff, power_curve)
    assert jnp.allclose(actual_power, expected_power)


def test_ws2aep_timeseries():
    ws_eff = jnp.array([[10.0, 12.0], [15.0, 20.0]])
    power_curve = jnp.array([[0.0, 0.0], [10.0, 1000.0], [25.0, 3000.0]])
    pc_xp, pc_fp = power_curve[:, 0], power_curve[:, 1]
    powers = jnp.array(
    ...
    actual_aep = ws2aep(ws_eff, power_curve)
    assert jnp.allclose(actual_aep, expected_aep)


def test_ws2aep_with_probabilities():
    ws_eff = jnp.array([[10.0, 12.0], [15.0, 20.0]])
    power_curve = jnp.array([[0.0, 0.0], [10.0, 1000.0], [25.0, 3000.0]])
    pc_xp, pc_fp = power_curve[:, 0], power_curve[:, 1]
    powers = jnp.array(
    ...
    actual_aep = ws2aep(ws_eff, power_curve, prob=probs)
    assert jnp.allclose(actual_aep, expected_aep)
