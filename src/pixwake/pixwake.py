...
# ------------------------
# 2) turbine-wake iteration via fixed_point
# ------------------------
def wake_step(a, ws_eff):
    ...
    mask = (x_d > 0) & (jnp.abs(dy) < wake_rad)
    ...
    term = 2 * a_coef * ((D / 2) / jnp.maximum(wake_rad, 1e-6)) ** 2

    # combine deficits in quadrature
    deficits = jnp.sqrt(jnp.sum(jnp.where(mask, term**2, 0.0), axis=1))
...
# ------------------------
# 3) example: gradient wrt turbine positions only
# ------------------------
